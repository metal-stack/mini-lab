...
.PHONY: up
up: bake env
	CURRENT_USER="$(shell id -u):$(shell id -g)" docker-compose up --remove-orphans --force-recreate control-plane partition && vagrant up machine01 machine02
...
partition: partition-bake
	CURRENT_USER="$(shell id -u):$(shell id -g)" docker-compose up --remove-orphans --force-recreate partition && vagrant up machine01 machine02
...
.PHONY: machine
machine: _privatenet
	docker-compose run metalctl machine create --description test --name test --hostname test --project 00000000-0000-0000-0000-000000000000 --partition vagrant --image ubuntu-20.04 --size v1-small-x86 --networks $(privatenet)

.PHONY: firewall
firewall: _ips _privatenet
	docker-compose run metalctl firewall create --description fw --name fw --hostname fw --project 00000000-0000-0000-0000-000000000000 --partition vagrant --image firewall-ubuntu-2.0 --size v1-small-x86 --networks internet-vagrant-lab,$(privatenet)
...
dev: cleanup caddy registry build-hammer-initrd build-api-image build-core-image push-core-image control-plane-bake load-api-image partition-bake env
	CURRENT_USER="$(shell id -u):$(shell id -g)" docker-compose -f docker-compose.yml -f docker-compose.dev.yml up
